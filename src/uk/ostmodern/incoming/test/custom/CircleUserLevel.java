...
//        this(context, attrs, com.mikhaellopez.circularimageview.R.attr.circularImageViewStyle);
    ...
    public CircleUserLevel(Context context, AttributeSet attrs, int defStyle) {
        ...
        filmsText = getContext().getResources().getString(R.string.films);
        ...
        filmsTextPaint = new Paint();
        filmsTextPaint.setTextAlign(Paint.Align.CENTER);
        filmsTextPaint.setColor(Color.BLACK);
//        filmsTextPaint.setColor(getContext().getResources().getColor(R.color.dark_gray_background_color));
        filmsTextPaint.setAntiAlias(true);
...
//        bigCircleRadius = ((borderWidth - (int)(distanceFromCircularImage * getDensity())) /2);
        ...
        for(int i = 0; i < drawingCounter;i++){
            ...
            if(filmsLevel < levelColorCounter && levelColorCounter != (levelColors.length - 1)){
                levelColorCounter = (levelColors.length - 1);
            }else if(filmsLevel == levelColorCounter && ((i % LEVELS_STEP) == (LEVELS_STEP - 1))){
                ...
                levelColorCounter++;
            }else if(filmsLevel > levelColorCounter && ((i % LEVELS_STEP) == (LEVELS_STEP - 1))) levelColorCounter++;
        ...
        if(levelCircleColor >= 0) { //draw labels and user level circle only if we have them
            ...
            filmsTextPaint.getTextBounds(Integer.toString(filmsCount), 0, Integer.toString(filmsCount).length(), textLevelBounds);
//            filmsTextPaint.getTextBounds(getUserLevelLikeString(), 0, Integer.toString(filmsLevel).length(), textLevelBounds);
            filmsTextPaint.setTextSize((bigCircleRadius * 4));

            if (showLevelLabel) {
                //draw first level label
                filmsTextPaint.getTextBounds(filmsText, 0, filmsText.length(), textLevelBounds);
                filmsTextPaint.setTextSize((bigCircleRadius * 2));
                canvas.drawText(filmsText, levelCircleXPosition, (levelCircleYPosition + filmsTextPaint.getTextSize()), filmsTextPaint);
                //after that draw lavel number with bold text
                filmsTextPaint.setTextSize((bigCircleRadius * 4));
                filmsTextPaint.setTypeface(Typeface.create(Typeface.DEFAULT, Typeface.BOLD));
                canvas.drawText(Integer.toString(filmsCount), levelCircleXPosition, levelCircleYPosition, filmsTextPaint);
//                canvas.drawText(getUserLevelLikeString(), levelCircleXPosition, levelCircleYPosition, filmsTextPaint);
            } else {
                filmsTextPaint.setTextSize((bigCircleRadius * 6));
                filmsTextPaint.setTypeface(Typeface.create(Typeface.DEFAULT, Typeface.BOLD));
                canvas.drawText(Integer.toString(filmsCount), levelCircleXPosition, levelCircleYPosition + (filmsTextPaint.getTextSize() / 4), filmsTextPaint);
//                canvas.drawText(getUserLevelLikeString(), levelCircleXPosition, levelCircleYPosition + (filmsTextPaint.getTextSize() / 4), filmsTextPaint);
            }
        }
...
//                    drawAllUserLevelCircles(canvas);
    ...
    public void setUserLevel(int filmsCount,boolean invalid) {
        this.filmsCount = (filmsCount < 0) ? 0 : filmsCount;
        if(filmsCount <= 1) filmsLevel = 0;
        else if((filmsCount / 5) >= 9) filmsLevel = 9;
        else filmsLevel = (filmsCount / 5);
//        filmsLevel = (level >= (levelColors.length -1)) ? ((levelColors.length -2)) : (level - 1);//we remove 1 because filmsLevel colors array
        if(invalid) invalidate();
    }
    public boolean isShowFilmsLabel(){
        return showLevelLabel;
    }
    public void isShowFilmsLabel(boolean show){
        showLevelLabel = show;
        invalidate();
    }
    public int getFilmsLevel() {
        return (filmsLevel +1);////we add 1 when return result because we want correct results
    }
    public String getUserLevelLikeString() {
        return Integer.toString(filmsLevel + 1);////we add 1 when return result because we want correct results
    }
//    @Override
...
//            if(listener != null){
//                this.bringToFront();
//                if(isProfileImageZoomIn) AnimateViewUtility.zoomOutProfileImage(this);
//                else AnimateViewUtility.zoomInProfileImage(this);
